...
\begin{proof}		
	...
	\begin{itemize}
		...
		\item $k^{++}^M = k^{++}^V$: Otherwise, let $\beta = k^{++}$ and $\gamma = k^{++}^M$.
			Then $\beta > \gamma$, so $j(\beta) > j(\gamma)$.
			Also let $g: \gamma \to k^+$ bijection. Then $j(g)$ is a bijection from $j(\gamma) \to j(k^{+})$ in $M$.
			Now consider $\phi(x,y) : x,y \text{ are cardinals and } y \text{ cardinal successor of } x$.
			Since $\phi(\k^{+},\beta)$ we have $(\phi(j(k^+),j(\beta))^M$.
			Since $\phi(\k,k^+)$ we have $(\phi(j(k),j(k^+))^M$.
			Therefore $M \models j(\beta) = j(k)^{++}$.

	\end{itemize}
	Now since in $V$ it is true that $2^{k} = k^{++}$ and all cardinals in the equation are the same in $M$, we deduce that the equation
	holds in $M$ as well.

	From $B \in U$ we deduce that $D = \{(s,A) : A \susbeteq B\}$ is dense.
	Then $G \cap D \not = \emptset$. Let $(s,A) \in G \cap D$.
